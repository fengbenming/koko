...
export default {
  ...
  data() {
    return {
      themes: [this.$t('Terminal.DefaultTheme'), ...themes],
      theme: window.localStorage.getItem("themeName") || this.$t('Terminal.DefaultTheme'),
    };
  },
  computed: {
    colors() {
  ...
  watch: {
    theme(val) {
      const theme = val && val !== this.$t('Terminal.DefaultTheme') ? val : '';
